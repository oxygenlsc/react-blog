...
export default function BlogEdit(props) {
    ...
    return (
        <div style={{ width: '100%', height: '100%' }}>
            <Button1 type='oneStyle' title='语法展开' top='80px' left='30px'
                click={showAndHide}
            />
            <Button2 type='oneStyle' title='上传图片' top='80px' left='120px' click={showImg} />
            <div className="edit-warpper" >
                <TextArea className="edit-inp-box" onChange={({ target: { value } }) => {
                    setinp(value)
                }}
                    onScroll={(e) => {
                ...
                <div className="show-html" id='md' ref={(e) => {
                    setsc(e)
                }}
                >
                    <ReactMarkdown source={inp} />
                </div>
                <div className="md-rule-show" style={{ opacity: rule ? 1 : 0, height: rule ? 500 : 0 }}>
                    # 这是一级标题
                    ## 这是二级标题
                    ### 这是三级标题
                    #### 这是四级标题
                ...
                <div className="send-img" style={{ opacity: imgsend ? 1 : 0, height: imgsend ? 500 : 0 }}>
                    ...
                    <ul className="urlList">
                        {myfileList.map((el,i)=>creatLi(el))}
                    </ul>
                </div>
            </div>
        </div>
...
function creatLi(props){
    return (<li className='url-list'>
            <div className="img-name">{props.name}</div>
            <Tooltip title='点击复制链接'>
            <div className="img-url">{props.url}</div>
            </Tooltip>
         </li>)
}